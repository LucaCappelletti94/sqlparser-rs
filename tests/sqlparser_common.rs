...
use test_utils::{
    ...
    call, expr_from_projection, join, number, only, parser_error, table, table_alias,
    table_from_name, TestedDialects,
};
...
fn parse_replace_into() {
    ...
    assert_eq!(
        ...
        "sql parser error: Unsupported statement REPLACE at Line: 1, Column: 9",
    )
}
...
fn parse_insert_default_values() {
    ...
    assert_eq!(
        parser_error("Expected: end of statement, found: PARTITION".to_string()),
...
fn parse_update() {
    ...
    assert_eq!(
        parser_error("Expected: SET, found: WHERE".to_string()),
        res.unwrap_err()
    );
    ...
    let res = parse_sql_statements(sql);
    assert_eq!(
        parser_error("Expected: end of statement, found: extrabadstuff".to_string()),
        res.unwrap_err()
    );
}
...
fn parse_delete_without_from_error() {
    ...
    assert_eq!(
        parser_error("Expected: FROM, found: WHERE".to_string()),
        res.unwrap_err()
    );
}
...
fn parse_outer_join_operator() {
    ...
    assert_eq!(
        parser_error("Expected: column identifier before (+), found: 2".to_string()),
        res.unwrap_err()
    );
}
...
fn parse_select_distinct_missing_paren() {
    ...
    assert_eq!(
        parser_error("Expected: ), found: FROM".to_string()),
        result.unwrap_err(),
    );
}
...
fn parse_select_all_distinct() {
    ...
    assert_eq!(
        parser_error("Cannot specify ALL then DISTINCT".to_string()),
        result.unwrap_err(),
    );
    let result = parse_sql_statements("SELECT DISTINCT ALL name FROM customer");
    assert_eq!(
        parser_error("Cannot specify DISTINCT then ALL".to_string()),
        result.unwrap_err(),
    );
...
fn parse_select_into() {
    ...
    assert_eq!(
        parser_error("Expected: end of statement, found: asdf".to_string()),
        result.unwrap_err()
    )
}
...
fn test_eof_after_as() {
    ...
    assert_eq!(
        parser_error("Expected: an identifier after AS, found: EOF".to_string()),
        res.unwrap_err()
    );
...
fn test_no_infix_error() {
    ...
    assert_eq!(
        parser_error("No infix parser for token ShiftLeft".to_string()),
        res.unwrap_err()
    );
}
...
fn parse_select_count_distinct() {
    ...
    let res = parse_sql_statements(sql);
    assert_eq!(
        parser_error("Cannot specify both ALL and DISTINCT".to_string()),
        res.unwrap_err()
    );
}
...
fn parse_in_error() {
    ...
    assert_eq!(
        parser_error("Expected: (, found: segment".to_string()),
        res.unwrap_err()
    );
}
...
fn parse_extract() {
    ...
    assert_eq!(
        parser_error("Expected: date/time field, found: JIFFY".to_string()),
        res.unwrap_err()
    );
}
...
fn parse_window_function_null_treatment_arg() {
    ...
    assert_eq!(
        ...
        parser_error("Expected: end of statement, found: NULLS".to_string())
    );
    ...
    assert_eq!(
        all_dialects_where(|d| !d.supports_window_function_null_treatment_arg())
            .parse_sql_statements(sql)
            .unwrap_err(),
        parser_error("Expected: ), found: IGNORE".to_string())
    );
}
...
fn parse_create_table_hive_array() {
    ...
    assert_eq!(
        dialects.parse_sql_statements(sql).unwrap_err(),
        parser_error("Expected: >, found: )".to_string())
    );
}
...
fn parse_rename_table() {
    ...
    assert_eq!(
        ...
        parser_error("Expected: end of statement, found: a".to_string())
    );

    assert_eq!(
        parse_sql_statements("RENAME TABLE1 old_table TO new_table a").unwrap_err(),
        parser_error("Expected: KEYWORD `TABLE` after RENAME, found: TABLE1".to_string())
    );
}
...
fn test_alter_table_with_on_cluster() {
    ...
    assert_eq!(
        res.unwrap_err(),
        parser_error("Expected: identifier, found: 123".to_string())
    )
}
...
fn parse_alter_table_alter_column_type() {
    ...
    assert_eq!(
        parser_error("Expected: end of statement, found: USING".to_string()),
        res.unwrap_err()
    );
}
...
fn parse_alter_table_drop_constraint() {
    ...
    assert_eq!(
        parser_error("Expected: end of statement, found: TEXT".to_string()),
        res.unwrap_err()
    );
}
...
fn parse_bad_constraint() {
    ...
    assert_eq!(
        parser_error("Expected: column name or constraint definition, found: EOF".to_string()),
        res.unwrap_err()
    );
}
...
fn parse_explain_query_plan() {
    ...
    assert_eq!(
        parser_error("Expected: end of statement, found: SELECT".to_string()),
        all_dialects()
...
fn parse_window_clause() {
    ...
    let res = dialects.parse_sql_statements(sql);
    assert_eq!(
        parser_error("Expected: (, found: window2".to_string()),
        res.unwrap_err()
    );
}
...
fn parse_interval_all() {
    ...
    assert_eq!(
        parser_error("Expected: end of statement, found: SECOND".to_string()),
        result.unwrap_err(),
    );
...
fn parse_table_function() {
    ...
    assert_eq!(
        parser_error("Expected: (, found: \'1\'".to_string()),
        res.unwrap_err()
    );

    let res = parse_sql_statements("SELECT * FROM TABLE (FUN(a) AS a");
    assert_eq!(
        parser_error("Expected: ), found: AS".to_string()),
        res.unwrap_err()
    );
}
...
fn parse_natural_join() {
    ...
    assert_eq!(
        parser_error("Expected: a join type after NATURAL, found: EOF".to_string()),
...
fn parse_join_syntax_variants() {
    ...
    assert_eq!(
        parser_error("Expected: APPLY, found: JOIN".to_string()),
        res.unwrap_err()
    );
}
...
fn parse_multiple_statements() {
    fn test_with(sql1: &str, sql2_kw: &str, sql2_rest: &str) {
        ...
        let res = parse_sql_statements(&(sql1.to_owned() + " " + sql2_kw + sql2_rest));
        assert_eq!(
            parser_error("Expected: end of statement, found: ".to_string() + sql2_kw),
            res.unwrap_err()
        );
    }
...
fn parse_overlay() {
    ...
    assert_eq!(
        parser_error("Expected: PLACING, found: FROM".to_owned()),
...
fn parse_trim() {
    ...
    assert_eq!(
        parser_error("Expected: ), found: 'xyz'".to_owned()),
    ...
    assert_eq!(
        parser_error("Expected: ), found: ,".to_owned()),
...
fn parse_drop_table() {
    ...
    assert_eq!(
        parser_error("Cannot specify both CASCADE and RESTRICT in DROP".to_string()),
        parse_sql_statements(sql).unwrap_err(),
    );
}
...
fn parse_invalid_subquery_without_parens() {
    ...
    assert_eq!(
        parser_error("Expected: end of statement, found: 1".to_string()),
        res.unwrap_err()
    );
}
...
fn lateral_derived() {
    ...
    let res = parse_sql_statements(sql);
    assert_eq!(
        parser_error("Expected: end of statement, found: WITH".to_string()),
        res.unwrap_err()
    );
...
fn parse_start_transaction() {
    ...
    assert_eq!(
        parser_error("Expected: isolation level, found: BAD".to_string()),
        res.unwrap_err()
    );

    let res = dialects.parse_sql_statements("START TRANSACTION BAD");
    assert_eq!(
        parser_error("Expected: end of statement, found: BAD".to_string()),
        res.unwrap_err()
    );

    let res = dialects.parse_sql_statements("START TRANSACTION READ ONLY,");
    assert_eq!(
        parser_error("Expected: transaction mode, found: EOF".to_string()),
        res.unwrap_err()
    );
...
fn parse_abort() {
    ...
    assert_eq!(
        ...
        parser_error("Expected: end of statement, found: TO".to_string()),
    );
}
...
fn test_merge_invalid_statements() {
    ...
    for (sql, err_msg) in [
        ...
        let res = dialects.parse_sql_statements(sql);
        assert_eq!(parser_error(err_msg.to_string()), res.unwrap_err());
    }
}
...
fn parse_offset_and_limit() {
    ...
    assert_eq!(
        parser_error("Expected: end of statement, found: OFFSET".to_string()),
        res.unwrap_err()
    );

    let res = parse_sql_statements("SELECT foo FROM bar LIMIT 2 LIMIT 2");
    assert_eq!(
        parser_error("Expected: end of statement, found: LIMIT".to_string()),
        res.unwrap_err()
    );
...
fn parse_cache_table() {
    ...
    assert_eq!(
        parser_error("Expected: a `TABLE` keyword, found: 'table_name'".to_string()),
        res.unwrap_err()
    );

    let res = parse_sql_statements("CACHE 'table_name' OPTIONS('K1'='V1')");
    assert_eq!(
        parser_error("Expected: a `TABLE` keyword, found: OPTIONS".to_string()),
        res.unwrap_err()
    );
...
fn parse_uncache_table() {
    ...
    assert_eq!(
        parser_error("Expected: TABLE, found: 'table_name'".to_string()),
        res.unwrap_err()
    );

    let res = parse_sql_statements("UNCACHE IF EXISTS 'table_name' foo");
    assert_eq!(
        parser_error("Expected: TABLE, found: IF".to_string()),
        res.unwrap_err()
    );
}
...
fn parse_trailing_comma() {
    ...
    assert_eq!(
        trailing_commas
            ...
            .unwrap_err(),
        parser_error("Expected an expression, found: from".to_string())
    );
    ...
    assert_eq!(
        trailing_commas
            ...
            .unwrap_err(),
        parser_error("Expected: a privilege keyword, found: ON".to_string())
    );
    ...
    assert_eq!(
        trailing_commas
            ...
            .unwrap_err(),
        parser_error("Expected: column name or constraint definition, found: )".to_string())
    );
...
fn test_create_policy() {
    ...
    assert_eq!(
        all_dialects()
            ...
            .unwrap_err()
            .to_string(),
        "sql parser error: Expected: CURRENT_USER, CURRENT_ROLE, SESSION_USER or identifier after OWNER TO, found: EOF"
    );
...
fn parse_listen_channel() {
    ...
    assert_eq!(
        ...
        parser_error("Expected: an SQL statement, found: LISTEN".to_string())
    );
}
...
fn parse_unlisten_channel() {
    ...
    assert_eq!(
        ...
        parser_error("Expected: wildcard or identifier, found: +".to_string())
    );
    ...
    assert_eq!(
        ...
        parser_error("Expected: an SQL statement, found: UNLISTEN".to_string())
    );
}
...
fn parse_notify_channel() {
    ...
    assert_eq!(
        dialects
            ...
            .unwrap_err(),
        parser_error("Expected: literal string, found: *".to_string())
    );
    ...
    for &sql in &sql_statements {
        assert_eq!(
            dialects.parse_sql_statements(sql).unwrap_err(),
            parser_error("Expected: an SQL statement, found: NOTIFY".to_string())
        );
    }
}
...
fn parse_load_data() {
    ...
    assert_eq!(
        only_supports_load_extension_dialects
            .parse_sql_statements(sql)
            .unwrap_err(),
        parser_error("Expected: end of statement, found: INPATH".to_string())
    );
    assert_eq!(
    ...
    assert_eq!(
        only_supports_load_extension_dialects
            .parse_sql_statements(sql)
            .unwrap_err(),
        parser_error("Expected: end of statement, found: LOCAL".to_string())
    );
    assert_eq!(
        not_supports_load_dialects
            .parse_sql_statements(sql)
...
fn parse_bang_not() {
    ...
    for &sql in &sql_statements {
        assert_eq!(
            dialects.parse_sql_statements(sql).unwrap_err(),
            parser_error("No infix parser for token ExclamationMark".to_string())
        );
    }
...
fn parse_comments() {
    ...
    assert_eq!(
        all_dialects_where(|d| d.supports_comment_on())
            ...
            .unwrap_err(),
        parser_error("Expected: IS, found: EOF".to_string())
    );
    ...
    assert_eq!(
        all_dialects_where(|d| d.supports_comment_on())
            ...
            .unwrap_err(),
        parser_error("Expected: comment object_type, found: UNKNOWN".to_string())
    );
}
...
fn parse_create_table_with_enum_types() {
    ...
    assert_eq!(
        all_dialects()
            ...
            .unwrap_err(),
        parser_error("Expected: literal string, found: 2".to_string())
    );
}
...
fn parse_update_from_before_select() {
    ...
    assert_eq!(
        parser_error("Expected: end of statement, found: FROM".to_string()),
...
fn parse_case_statement() {
    ...
    assert_eq!(
        parser_error("Expected: THEN, found: END".to_string()),
        parse_sql_statements("CASE 1 WHEN a END").unwrap_err()
    );
    assert_eq!(
        parser_error("Expected: WHEN, found: ELSE".to_string()),
...
fn parse_if_statement() {
    ...
    assert_eq!(
        parser_error("Expected: IF, found: EOF".to_string()),
        dialects
...
fn parse_raise_statement() {
    ...
    assert_eq!(
        parser_error("Expected: =, found: error".to_string()),
...
fn parse_pipeline_operator_negative_tests() {
    ...
    assert_eq!(
        parser_error("EXCEPT pipe operator requires DISTINCT modifier".to_string()),
        dialects
    ...
    assert_eq!(
        parser_error("INTERSECT pipe operator requires DISTINCT modifier".to_string()),
        dialects
...
fn test_select_exclude() {
    ...
    assert_eq!(
        dialects
            ...
            .err()
            .unwrap(),
        parser_error("Expected: end of statement, found: c2".to_string())
    );
    ...
    assert_eq!(
        dialects
            ...
            .err()
            .unwrap(),
        parser_error("Expected: end of statement, found: EXCLUDE".to_string())
    );
}
...
fn parse_odbc_time_date_timestamp() {
    ...
    assert_eq!(
        parser_error("Expected: :, found: '14:12:01'".to_string()),
        res.unwrap_err()
    );
    assert_eq!(
        parser_error("Expected: an expression, found: {".to_string()),
...
fn parse_create_view_if_not_exists() {
    ...
    assert_eq!(
        parser_error("Expected: AS, found: SELECT".to_string()),
        res.unwrap_err()
    );
}
...
fn parse_table_preserves_quotes_and_trailing_tokens() {
    ...
    let err = backtick_dialects
        ...
        .unwrap_err();
    assert_eq!(
        parser_error("Expected: end of statement, found: ORE".to_string()),
        err
    );
}
...
#[test]
fn parse_error_at_end_of_input_reports_location() {
    let dialects = all_dialects();
    assert_eq!(
        dialects
            .parse_sql_statements_with_locations("SELECT 1 FROM t WHERE")
            .unwrap_err()
            .to_string(),
        "sql parser error: Expected: an expression, found: EOF at Line: 1, Column: 22"
    );
    assert_eq!(
        dialects
            .parse_sql_statements_with_locations("SELECT 1\nFROM t\nWHERE\n-- trailing\n")
            .unwrap_err()
            .to_string(),
        "sql parser error: Expected: an expression, found: EOF at Line: 3, Column: 6"
    );
    for sql in ["", " \n "] {
        let err = dialects.one_of_identical_results(|dialect| {
            Parser::new(dialect)
                .try_with_sql(sql)
                .unwrap()
                .parse_expr()
        ...
        assert_eq!(
            err.to_string(),
            "sql parser error: Expected: an expression, found: EOF at Line: 1, Column: 1"
        );
    }
}
...
fn parse_error_span() {
    let dialect = GenericDialect {};
    let err = Parser::parse_sql(&dialect, "SELECT 1 FROM t x y").unwrap_err();
    assert_eq!(
        err.to_string(),
        "sql parser error: Expected: end of statement, found: y at Line: 1, Column: 19"
    );
    assert_eq!(
        err.span(),
        Span::new(Location::new(1, 19), Location::new(1, 20))
    );

    let err = Parser::parse_sql(&dialect, "SELECT 1 FROM t WHERE").unwrap_err();
    assert_eq!(
        err.span(),
        Span::new(Location::new(1, 22), Location::new(1, 22))
    );

    let err = Parser::parse_sql(&dialect, "SELECT 'abc").unwrap_err();
    assert_eq!(
        err.to_string(),
        "sql parser error: Unterminated string literal at Line: 1, Column: 8"
    );
    assert_eq!(
        err.span(),
        Span::new(Location::new(1, 8), Location::new(1, 8))
    );
}

#[test]
fn parse_recursion_limit_error_reports_location() {
    let err = Parser::new(&GenericDialect {})
    ...
    assert_eq!(
        err.to_string(),
        "sql parser error: recursion limit exceeded at Line: 1, Column: 9"
    );
    assert_eq!(
        err,
...
#[test]
fn parse_extract_missing_from_reports_location() {
    assert_eq!(
        all_dialects()
            .parse_sql_statements_with_locations("SELECT EXTRACT(YEAR x)")
            .unwrap_err()
            .to_string(),
        "sql parser error: Expected: 'FROM' or ',', found: x at Line: 1, Column: 21"
    );
}
